...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target, epoch):
        ...
        loss_ys   = list()
        for i in range(num_keypoints):
            lx = self.coord_scale * nn.MSELoss(size_average=False)(x[i]*coord_mask, txs[i]*coord_mask)/2.0
            ly = self.coord_scale * nn.MSELoss(size_average=False)(y[i]*coord_mask, tys[i]*coord_mask)/2.0
            loss_xs.append(lx)
            loss_ys.append(ly)
        loss_conf  = nn.MSELoss(size_average=False)(conf*conf_mask, tconf*conf_mask)/2.0
        #loss_x    = np.sum(loss_xs)
        #loss_y    = np.sum(loss_ys)
        loss_x    = torch.stack(loss_xs, dim=0).sum(dim=0).sum(dim=0)
        loss_y    = torch.stack(loss_ys, dim=0).sum(dim=0).sum(dim=0)
